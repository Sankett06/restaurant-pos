...
export default function Dashboard() {
  ...
  const stats = [
    ...
    {
      title: 'Active Orders',
      value: state.orders.filter(o => ['pending', 'preparing'].includes(o.status)).length.toString(),
      change: '+3',
      icon: ShoppingCart,
      color: 'text-blue-600',
      bgColor: 'bg-blue-50',
    },
    {
      ...
      value: state.tables.filter(t => t.status === 'occupied').length.toString(),
      change: `/${state.tables.length}`,
      icon: Users,
    ...
    {
      title: 'KOT Pending',
      value: state.kots.filter(k => k.status === 'pending').length.toString(),
      change: 'urgent',
      icon: ChefHat,
      color: 'text-red-600',
      bgColor: 'bg-red-50',
